...
const url_avatar = ref(baseUrl + '/usuario/' + 1 + '/fotoperfil');

onMounted(() => {

  url_avatar.value = (baseUrl + '/usuario/' + 1 + '/fotoperfil');
 // url_avatar.value = baseUrl + '/usuarios/' + authStore.user.user_info.id + '/fotoperfil';
  //url_avatar.value = baseUrl + '/usuarios/' + authStore.user.user_info.id + '/fotoperfil';
 //  , console.log(url_avatar);
});

const showSuccessAlert = ref(true)
const showTarefaRapida = ref(false)
...
<template>
  <v-app-bar elevation="0" :priority="priority" height="80" style=" overflow: visible !important; ">
    ...
    <v-menu :close-on-content-click="false">
      <template v-slot:activator="{ props }">
        <v-btn @click="showTarefaRapida = true" icon class="text-secondary mx-3" color="lightsecondary" rounded="sm"
          size="small" variant="flat" v-bind="props">
    ...
    <v-menu :close-on-content-click="false">
      <template v-slot:activator="{ props }">
        <v-btn class="profileBtn text-primary" color="lightprimary" variant="flat" rounded="pill" v-bind="props">
          <v-avatar size="35" class="mr-2  rounded-full ">
            <v-img alt="Avatar" :src="url_avatar" ></v-img>
          </v-avatar>
